...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 10 09:51:31 2024
...
# Den *= c.den_converter
Num_den = Den / Mass
Num_den *= c.numden_converter

# Caclculate Ks
# NOTE: Add degeneracy factors
lamda_th = c.h / np.sqrt(2 * np.pi * c.me * c.Kb * T)
A1 = 2 / (Num_den * lamda_th**3 ) * c.prefactor_h *  np.exp(-c.xh/(c.kb * T))
A2 = 2 / (Num_den * lamda_th**3 ) * c.prefactor_he1 *  np.exp(-c.xhe1/(c.kb * T))
A3 = 2 / (Num_den * lamda_th**3 ) * c.prefactor_he2 *  np.exp(-c.xhe2/(c.kb * T))
...
ion1 = np.zeros(len(A1))
ion2 = np.zeros(len(A1))
ion3 = np.zeros(len(A1))
for i in tqdm(range(len(A1))):
    ion1[i] = fsolve(saha, 0.5, args = A1[i], fprime = sahaprime)
    ion2[i] = fsolve(saha, 0.5, args = A2[i], fprime = sahaprime)
    ion3[i] = fsolve(saha, 0.5, args = A3[i], fprime = sahaprime)
...
plt.ioff()
for check in tqdm( range(5, 100)):#ray_no) ):
