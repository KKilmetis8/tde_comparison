...
def loader(simname, fixes):
    spectra = []
    for fix in fixes:
        temp_spectra = np.loadtxt(f'{simname}sumthomp3_6spectra{fix}.txt')
...
def plotter(spectra, title):
    ...
    for i, obs in enumerate(np.linspace(0, 191, 20)):
        print(spectra)
        obs = int(obs)
        # ax.plot(freqs, freqs * spectra[cardinal[i]], c = colors[i], lw = 2,
        #         label = labels[i])
        ax.plot(freqs, freqs * spectra[obs], lw = 2, c = c.r20_palette[i],
                label = str(obs))#labels[i])
        if spectra[obs][446]*freqs[446] > 1e40:
            print(obs)
            print(spectra[obs][446])
    ax.legend(bbox_to_anchor = (1.2,0.1,0.1,1), ncols = 2, frameon = False)
    ax.set_xscale('log')
    # ax.set_yscale('log')
...
m = 6
simname = f'data/blue2/spectra{m}new/'
if Mbh == 10_000:
    if 'HiRes' in extra:
        fixes = [161, 210]
