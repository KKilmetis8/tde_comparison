function TDE_find_photo_new(nstart, nend)
    display(nstart)
    display(nend)
    if ischar(nstart)
    ...
    for j=nstart:1:nend %625
       ...
       outdir=dirname;
        ...
        j
        disp('Start reading');
        res=readhdf3d(sprintf('%s/snap_full_%d.h5',dirname,j));
        ...
        for i=1:length(out)
            ...
            % ind = d < exp(Rhointerp(1)) & t < exp(Tinterp(1));
            % if(sum(ind) > 0)
            %     d_slope = (rossland(1, 7) - rossland(1, 1)) / (Rhointerp(7) - Rhointerp(1));
			% 	T_slope = (rossland(7, 1) - rossland(1, 1)) / (Tinterp(7) - Tinterp(1));
            %     sigma_rossland(ind) = exp(rossland(1, 1) + d_slope * (log(d(ind)) -Rhointerp(1)) + T_slope * (log(t(ind)) - Tinterp(1)));
            % end
            % ind = d > exp(Rhointerp(end)) & t < exp(Tinterp(1));
            % if(sum(ind) > 0)
            %     d_slope = (rossland(1, end) - rossland(1, end - 7)) / (Rhointerp(end) - Rhointerp(end - 7));
			% 	T_slope = (rossland(7, end) - rossland(1, end)) / (Tinterp(7) - Tinterp(1));
            %     sigma_rossland(ind) = exp(rossland(1, end) + d_slope * (log(d(ind)) -Rhointerp(1)) + T_slope * (log(t(ind)) - Tinterp(1)));
            % end
            % ind = d < exp(Rhointerp(1)) & t > exp(Tinterp(end));
            % if(sum(ind) > 0)
            %     d_slope = (rossland(end, 7) - rossland(end, 1)) / (Rhointerp(7) - Rhointerp(1));
            %     sigma_rossland(ind) = exp(rossland(end, 1) + d_slope * (log(d(ind)) -Rhointerp(1)));
            % end
            % ind = d > exp(Rhointerp(end)) & t > exp(Tinterp(end));
            % if(sum(ind) > 0)
            %     d_slope = (rossland(end, end) - rossland(end, end - 7)) / (Rhointerp(end) - Rhointerp(end - 7));
            %     sigma_rossland(ind) = exp(rossland(end, end) + d_slope * (log(d(ind)) -Rhointerp(1)));
            % end
            % ind = d < exp(Rhointerp(1)) & t < exp(Tinterp(end)) & t > exp(Tinterp(1));
            % if(sum(ind) > 0)
            %     d0 = interp2(Tinterp,Rhointerp,rossland',log(t), ones(sum(ind),1).*Rhointerp(1)*0.9999,'linear');
            %     d_slope = (interp2(Tinterp,Rhointerp,rossland',log(t), ones(sum(ind),1).*Rhointerp(7),'linear') - d0) / (Rhointerp(7) - Rhointerp(1));
            %     sigma_rossland(ind) = exp(d0 + d_slope * (log(d(ind)) - Rhointerp(1)));
            % end
            % ind = d > exp(Rhointerp(end)) & t < exp(Tinterp(end)) & t > exp(Tinterp(1));
            % if(sum(ind) > 0)
            %     d0 = interp2(Tinterp,Rhointerp,rossland',log(t), ones(sum(ind),1).*Rhointerp(end)*0.9999,'linear');
            %     d_slope = (interp2(Tinterp,Rhointerp,rossland',log(t), ones(sum(ind),1).*Rhointerp(end - 7),'linear') - d0) / (Rhointerp(end - 7) - Rhointerp(end));
            %     sigma_rossland(ind) = exp(d0 + d_slope * (log(d(ind)) - Rhointerp(end)));
            % end
            % 
            % 
            % 
            % 
            % sigma_planck=exp(interp2(Tinterp,Rhointerp,planck',log(t),log(d),'linear',0));
            % ind = d < exp(Rhointerp(1)) & t < exp(Tinterp(1));
            % if(sum(ind) > 0)
            %     d_slope = (planck(1, 7) - planck(1, 1)) / (Rhointerp(7) - Rhointerp(1));
			% 	T_slope = (planck(7, 1) - planck(1, 1)) / (Tinterp(7) - Tinterp(1));
            %     sigma_planck(ind) = exp(planck(1, 1) + d_slope * (log(d(ind)) -Rhointerp(1)) + T_slope * (log(t(ind)) - Tinterp(1)));
            % end
            % ind = d > exp(Rhointerp(end)) & t < exp(Tinterp(1));
            % if(sum(ind) > 0)
            %     d_slope = (planck(1, end) - planck(1, end - 7)) / (Rhointerp(end) - Rhointerp(end - 7));
			% 	T_slope = (planck(7, end) - planck(1, end)) / (Tinterp(7) - Tinterp(1));
            %     sigma_planck(ind) = exp(planck(1, end) + d_slope * (log(d(ind)) -Rhointerp(end)) + T_slope * (log(t(ind)) - Tinterp(1)));
            % end
            % 
            % ind = d < exp(Rhointerp(1)) & t > exp(Tinterp(end));
            % if(sum(ind) > 0)
            %     d_slope = (planck(end, 7) - planck(end, 1)) / (Rhointerp(7) - Rhointerp(1));
            %     sigma_planck(ind) = exp(planck(end, 1) + d_slope * (log(d(ind)) -Rhointerp(1)) -3.5 * (log(t(ind)) - Tinterp(end)));
            % end
            % ind = d > exp(Rhointerp(end)) & t > exp(Tinterp(end));
            % if(sum(ind) > 0)
            %     d_slope = (planck(end, end) - planck(end, end - 7)) / (Rhointerp(end) - Rhointerp(end - 7));
            %     sigma_planck(ind) = exp(planck(end,end) + d_slope * (log(d(ind)) -Rhointerp(end)) -3.5  * (log(t(ind)) - Tinterp(end)));
            % end
