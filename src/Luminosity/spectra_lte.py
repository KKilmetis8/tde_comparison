...
def luminosity_n(Temperature: float, Density: float, tau: float, volume: float, n: float):
    ...
    Tmax = np.exp(17.87) #TEST OLD OPACITY
    if Temperature > Tmax:
...
def dot_prod(xyz_selected, thetas, phis):
    ...
    dot_product *= 4/192 # normalisation
    return dot_product
...
if __name__ == "__main__":
    ...
    for idx_sn in range(-1,0): #so you take snap 881
        ...
        for idx in range(len(wanted_thetas)):
            ...
            with open(f'{pre_saving}lte_nLn_single_m{m}_{snap}.txt', 'a') as fselect:
